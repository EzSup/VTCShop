...
namespace VTCShop.Infrastructure.Services
{
    public class FileStorageRepository
    {
        ...
        public FileStorageRepository(IAmazonS3 s3Client, IOptions<S3Options> options, 
        ...
        public async Task<string> UploadFileFromStreamAsync(Stream stream, string? folderName, string? fileName)
        {
            var newName = Guid.NewGuid().ToString().Replace("-", "").Substring(0, 16);
            var filePath = fileName ?? newName;
            var key = string.IsNullOrEmpty(folderName) ? filePath : $"{folderName}/{filePath}";
        ...
        {
            return await _s3Client.GetObjectStreamAsync(_options.S3BucketName, key, new Dictionary<string, object>());
        }
